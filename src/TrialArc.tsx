 ...
 * (ask, save, first PAKKA, landed) as stars that light up.
 *
...
function copy(language: string) {
  if (language === "Hindi")
    return {
      day: (d: number) => `Free week · दिन ${d}/7`,
      ask: "पहला सवाल",
      save: "Lines save",
      pakka: "पहला PAKKA",
      landed: "Landed",
      keep: (n: number) => `आपके ${n} concepts हर plan पर साथ रहेंगे।`,
    };
  if (language === "English")
    return {
  ...
  return {
    day: (d: number) => `Free week · din ${d}/7`,
    ask: "Pehla sawaal",
    save: "Save lines",
    pakka: "Pehla PAKKA",
    landed: "Landed",
    keep: (n: number) => `Aapke ${n} concepts har plan par saath rahenge.`,
  };
}
...
export function TrialArc({
  ...
  const stars: ArcStar[] = [
    ...
    ...(landingEnabled
      ? [
          { key: "pakka", label: c.pakka, done: wins > 0 },
